...
import org.fog.pfe.bss.PrintConfig; 


public class FogDevice extends PowerDatacenter {
	protected Queue<Tuple> northTupleQueue_1;
	protected Queue<Tuple> northTupleQueue_2;
	protected Queue<Tuple> northTupleQueue_3;
	protected Queue<Tuple> northTupleQueue_4;
	...
	protected boolean DROP1 = false;
	protected boolean DROP2 = false;
	protected boolean DROP3 = false;
	protected boolean DROP4 = false;
	long queueCapacity = 400; 
	
	protected long load = 0;
	protected double sendingEnergyConsumption=0;
	protected final double sendingConsumptionRate = 0.01 ; //rate of consumption of sending one octet : en jul 
	
	protected long bandwidthConsumption=0;
	...
	public FogDevice(
		...
		northTupleQueue_1 = new LinkedList<Tuple>();
		northTupleQueue_2 = new LinkedList<Tuple>();
		northTupleQueue_3 = new LinkedList<Tuple>();
		northTupleQueue_4 = new LinkedList<Tuple>();
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		case FogEvents.TUPLE_ARRIVAL:
			...
			//if(ev.getSource() != ev.getDestination())
				...
				//System.out.println(ev+" tuple = "+((Tuple)ev.getData()).getTupleType()+" tuple priority : "+((Tuple)ev.getData()).getPriority());
			if ( ( this.getLevel() == PrintConfig.LEVEL_PRINT ) && ( ((Tuple)ev.getData()).getPriority() == PrintConfig.PACKET_PRIORITY_PRINT ) )
				PrintConfig.setPacketCounter();
	...
	private void manageResources(SimEvent ev) {
		if(getName().equals("cloud")){
			System.out.print((int)(CloudSim.clock()/Config.MAX_SIMULATION_TIME * 100)+"% ");
			if(CloudSim.clock() == Config.MAX_SIMULATION_TIME)
				System.out.print("\n");
		}
	...
	protected void updateNorthTupleQueue(){
		//if(!getNorthTupleQueue().isEmpty()) drop(PrintConfig.DROP_LEVEL, PrintConfig.DROP_PRIORITY);
		
		if(!getNorthTupleQueue().isEmpty()){
			Tuple tuple = getNorthTupleQueue().poll();
			this.load=this.load + tuple.getCloudletFileSize();
	...
	*/
	...
	protected void updateNorthTupleQueue(){
		...
		if (this.DROP) {
		while(!getNorthTupleQueue().isEmpty()){
			tuple = getNorthTupleQueue().poll();
			if (! (tuple.getPriority() == PrintConfig.DROP_PRIORITY)) {this.load=this.load + tuple.getCloudletFileSize(); sendUpFreeLink(tuple); }
			}
		}else{
			if(!getNorthTupleQueue().isEmpty()){
				tuple = getNorthTupleQueue().poll();
...
protected void updateNorthTupleQueue(){
		...
		//prio 1
		while(!getNorthTupleQueue1().isEmpty()){
			if (this.DROP1) {
			tuple = getNorthTupleQueue1().poll();
			if (! (tuple.getPriority() == PrintConfig.DROP_PRIORITY)) {this.load=this.load + tuple.getCloudletFileSize(); sendUpFreeLink(tuple); }
			}else{
				tuple = getNorthTupleQueue1().poll();
				this.load=this.load + tuple.getCloudletFileSize();
		...
		//prio 2
		while(!getNorthTupleQueue2().isEmpty()){
			if (this.DROP2) {
			tuple = getNorthTupleQueue2().poll();
			if (! (tuple.getPriority() == PrintConfig.DROP_PRIORITY)) {this.load=this.load + tuple.getCloudletFileSize(); sendUpFreeLink(tuple); }
			}else{
				tuple = getNorthTupleQueue2().poll();
				this.load=this.load + tuple.getCloudletFileSize();
		...
		//prio 3
		while(!getNorthTupleQueue3().isEmpty()){
			if (this.DROP3) {
			tuple = getNorthTupleQueue3().poll();
			if (! (tuple.getPriority() == PrintConfig.DROP_PRIORITY)) {this.load=this.load + tuple.getCloudletFileSize(); sendUpFreeLink(tuple); }
			}else{
				tuple = getNorthTupleQueue3().poll();
				this.load=this.load + tuple.getCloudletFileSize();
			...
			//prio 4
			while(!getNorthTupleQueue4().isEmpty()){
				if (this.DROP4) {
				tuple = getNorthTupleQueue4().poll();
				if (! (tuple.getPriority() == PrintConfig.DROP_PRIORITY)) {this.load=this.load + tuple.getCloudletFileSize(); sendUpFreeLink(tuple); }
				}else{
					tuple = getNorthTupleQueue4().poll();
					this.load=this.load + tuple.getCloudletFileSize();
	...
	protected void sendUpFreeLink(Tuple tuple){
		//TODO sendUpFreeLink
		if (this.getName().contains("OBJ")) {tuple.setPriority(allocateTuplePriority()); }
		//if (this.getLevel() == (PrintConfig.NUM_OF_LEVEL + 1)) tuple.setPriority(allocateTuplePriority());
		
		
		//if (!filtrage(PrintConfig.DROP_LEVEL ,PrintConfig.DROP_PRIORITY ,tuple.getPriority())) {
		
		this.sendingEnergyConsumption=this.sendingEnergyConsumption + (tuple.getCloudletFileSize() * sendingConsumptionRate); //consomation d'energie 
		this.bandwidthConsumption=this.bandwidthConsumption + tuple.getCloudletFileSize(); //consomation de la bande passante
	...
	protected void sendUp(Tuple tuple){
		if(parentId > 0){
			if(!isNorthLinkBusy()){
				...
				int prio = tuple.getPriority();
				
				switch (prio) {
				case 0:
					getNorthTupleQueue1().add(tuple);
					if (getQueueOccupation(getNorthTupleQueue1()) > 90) { this.DROP1 = true; }
					break;
				case 1:
					getNorthTupleQueue2().add(tuple);
					if (getQueueOccupation(getNorthTupleQueue2()) > 80) { this.DROP2 = true; }
					break;
				case 2:
					getNorthTupleQueue3().add(tuple);
					if (getQueueOccupation(getNorthTupleQueue3()) > 70) { this.DROP3 = true; }
					break;
				case 3:
					getNorthTupleQueue4().add(tuple);
					if (getQueueOccupation(getNorthTupleQueue4()) > 50) { this.DROP4 = true; }
					break;
				}
	...
	public Queue<Tuple> getNorthTupleQueue1() {
		return northTupleQueue_1;
	}
	
	public Queue<Tuple> getNorthTupleQueue2() {
		return northTupleQueue_2;
	}
	public Queue<Tuple> getNorthTupleQueue3() {
		return northTupleQueue_3;
	}
	
	public Queue<Tuple> getNorthTupleQueue4() {
		return northTupleQueue_4;
	}

	public void setNorthTupleQueue1(Queue<Tuple> queue) {
		this.northTupleQueue_1 = queue;
	}
	
	public void setNorthTupleQueue2(Queue<Tuple> queue) {
		this.northTupleQueue_2 = queue;
	}
	
	public void setNorthTupleQueue3(Queue<Tuple> queue) {
		this.northTupleQueue_3 = queue;
	}
	
	public void setNorthTupleQueue4(Queue<Tuple> queue) {
		this.northTupleQueue_4 = queue;
	}
	...
	public double getSendingEnergyConsumed() {
		return sendingEnergyConsumption;
	}
	...
	//*****************************************************************
	private int allocateTuplePriority() {
		
		int percentage = (int)((Math.random() * (10-1)) + 1);
		
		if (percentage == 1 ) return 0;
		else if (percentage < 4) return 1;
		else if (percentage < 7) return 2;
		else return 3;
	}

	protected boolean filtrage(int level, int priority, int tuplePriority) {
		
		if ( ( this.level==level ) && ( priority==tuplePriority ) ) return true;
		else return false;
	}
	/*
	protected void drop(int level, int priority) {
		
		 Queue<Tuple> queueTemp =  new LinkedList<>();
		 Tuple tuple;
		 
		if ( this.level==level ) {
			while (!getNorthTupleQueue().isEmpty()) {
				tuple = getNorthTupleQueue().poll();
				if (tuple.getPriority() != priority) queueTemp.add(tuple);
			}
			
			while (!queueTemp.isEmpty()) {
				tuple = queueTemp.poll();
				getNorthTupleQueue().add(tuple);
			}
		}
		
	}
	*/
	private double getQueueOccupation(Queue<Tuple> file) {
		
		double pourcentage;
		
		//calcul de la charge de noeud
		long Q = 0;
		for (Tuple tpl : file) {
			Q = Q + tpl.getCloudletFileSize();
		}
		
		
		
		pourcentage = (Q * 100) / this.queueCapacity;
